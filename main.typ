...
code, i.e., function closures for accessing a shared
resource, are automatically wrapped with instructions that
stack and update the system ceiling using a single,
predefined value based on the resource. The targets
supported by RTIC must support prioritized interrupts and
interrupt masking which is used to create a hardware
