...
(On connection between RTIC and SRP, work in progress)

In SRP,
$
macron(pi) = max{ceil(R)}
$<eq:1>
...
$
ceil(R) = \
max({0} union {pi(t_"cur")} union \
{pi(t) mid(|) t "needs more" R "than current amount of" R})
$<eq:2>

Which means
$
=>^(#ref(<eq:1>), #ref(<eq:2>), #ref(<eq:3>)) \
macron(pi)_"initial" = 0
$<eq:3>
It also means
$
=>^(#ref(<eq:1>), #ref(<eq:2>), #ref(<eq:3>)) \
"after a mutex lock on R" \
macron(pi) = max(macron(pi)_"old", ceil(R)_v_R),
$
where $v_R$ is the current amount of R---or in RTIC terms,
$
macron(pi) = max(macron(pi)_"old", pi(R)).
$
In SRP, when a new task $t$ is started,
$
macron(pi) = max(macron(pi)_"old", pi(t)),
$
but in RTIC, higher priority task can never preempt a lower priority task, it in enough to change the system ceiling only when a lock is taken.
